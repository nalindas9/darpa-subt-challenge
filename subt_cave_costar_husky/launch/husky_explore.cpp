...
float c = 1.0;
float a = 5.0;
float b = 1.1; // larger b allows closer obstacles  2.0
...
void laser_odom_callback(const nav_msgs::Odometry::ConstPtr &msg)
{
    float curr_x = msg->pose.pose.position.x;
    float curr_y = msg->pose.pose.position.y;
    float diff = sqrt((curr_x - prev_x) * (curr_x - prev_x) + (curr_y - prev_y) * (curr_y - prev_y));
    std::cout << "dx:" << diff << std::endl;
    if (diff <= 0.01) // robot is not moving
        static_counter++;
    else
        static_counter = 0;

    if (static_counter > 20)
    {
        std::cout << "getting stuck" << std::endl;
        RECOVERY = 1;
        static_counter = 0;
    }
    prev_x = curr_x;
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber laser_odom_sub = n.subscribe("COSTAR_HUSKY/laser_odom", 1, laser_odom_callback);
